...
rules:
  - name:           # (string) Human-friendly rule name
    ...
    character_requirements:         # (optional) character type requirements
...
| character_requirements  | Require specific character types (digits, uppercase, lowercase, special) |
...
The `character_requirements` field allows you to specify data type requirements for matched secrets. This is particularly useful when:
...
Kingfisher's regex engine (Hyperscan) does not support lookahead assertions like `(?=.*\d)` to require specific character types. Instead, use the `character_requirements` field to filter matches post-detection.
...
- The `character_requirements` filters out matches that don't have at least one of each required type
